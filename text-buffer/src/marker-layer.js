...
(MarkerLayer = class MarkerLayer {
  ...
  static deserializeSnapshot (snapshot) {
    ...
    for (const layerId in snapshot) {
      ...
      result[layerId] = {}
      for (const markerId in markerSnapshots) {
  ...
  */
  ...
  copy () {
    ...
    for (const markerId in this.markersById) {
  ...
  */
  ...
  getMarkers () {
    return (() => {
      const result = []
      for (const id in this.markersById) {
  ...
  */
  ...
  createSnapshot () {
    ...
    for (const id of Array.from(Object.keys(this.markersById))) {
      const marker = this.markersById[id]
  ...
  deserialize (state) {
    ...
    for (const id in state.markersById) {
