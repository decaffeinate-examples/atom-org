...
var YouTube = (function() {
  let INFO_URL = undefined;
  let VIDEO_EURL = undefined;
  let HEADER_SIZE = undefined;
  let ytid = undefined;
...
|youtu\\.be\\/)([^"&?\\/]{11})`, 'i');
    ...
    getMap(map){
      ...
      for (let i = 0; i < streamMap.length; i++) {
        ...
        for (let data of Array.from(streamData)) {
    ...
    getVideoInfo(url,next){
      ...
      return request(reqUrl,(err,response,body)=> {
        ...
        this.formats = {};
        for (let adaptive of Array.from(this.adaptiveStreams)) {
          const {
            itag
          } = adaptive;
      ...
      if (chunk != null) {
       ...
       //console.log 'host',host[1]
        ...
        return request(reqObj,(err,res,body)=> {
          ...
          for (i = 0, end = body.length, asc = 0 <= end; asc ? i <= end : i >= end; asc ? i++ : i--) {
    ...
    makeNewHeader(next){
      ...
      let y = 0;
      for (let index of Array.from(this.downloadIndexes)) {
     ...
     //console.log 'copying all header data'
      // full header clone
      for (i = 0, end1 = newHeaderSize-1, asc1 = 0 <= end1; asc1 ? i <= end1 : i >= end1; asc1 ? i++ : i--) {
      ...
      for (i = 0, end2 = newRefsSize-1, asc2 = 0 <= end2; asc2 ? i <= end2 : i >= end2; asc2 ? i++ : i--) {
     ...
     //console.log 'tkhd,mvhd,mdhd',tkhd,mvhd,mdhd
      ...
      let newDuration = 0;
      for (let chunk of Array.from(this.chunksToDownload)) {
     ...
     //console.log 'init',url
      ...
      return request(reqObj,(err,res,body)=> {
       ...
       //console.log 'timescale',@timescale
        ...
        let time = 0;
        for (let reference of Array.from(this.references)) {
