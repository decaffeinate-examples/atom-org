...
(PackageConverter = class PackageConverter {
  ...
  convertSettings (settings) {
    if (settings.shellVariables) {
      ...
      for (const { name, value } of Array.from(settings.shellVariables)) {
  ...
  normalizeFilenames (directoryPath) {
    ...
    return (() => {
      const result = []
      for (const child of Array.from(fs.readdirSync(directoryPath))) {
  ...
  convertSnippets (packageName, source) {
    ...
    for (const child of Array.from(fs.readdirSync(sourceSnippets))) {
      var left, selector
  ...
  convertPreferences (packageName, source) {
    ...
    for (const child of Array.from(fs.readdirSync(sourcePreferences))) {
      var left, properties
  ...
  convertGrammars (source) {
    ...
    for (const child of Array.from(fs.readdirSync(sourceSyntaxes))) {
