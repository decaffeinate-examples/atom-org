...
(Rule = class Rule {
  constructor (grammar, registry, param) {
    ...
    this.patterns = []
    for (const pattern of Array.from(patterns != null ? patterns : [])) {
  ...
  scanInjections (ruleStack, line, position, firstLine) {
    ...
    if (injections = baseGrammar.injections) {
      for (const scanner of Array.from(injections.getScanners(ruleStack))) {
  ...
  findNextMatch (ruleStack, lineWithNewline, position, firstLine) {
    ...
    if (result = this.scanInjections(ruleStack, lineWithNewline, position, firstLine)) {
      for (const injection of Array.from(baseGrammar.injections.injections)) {
    ...
    let scopes = null
    for (const injectionGrammar of Array.from(this.registry.injectionGrammars)) {
