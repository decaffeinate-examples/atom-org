...
module.exports = function ({ blobStore }) {
  ...
  const { resourcePath, devMode, env } = getWindowLoadSettings()
  ...
  // Make React faster
  if (!devMode) { if (process.env.NODE_ENV == null) { process.env.NODE_ENV = 'production' } }
  ...
  return global.atom.startEditorWindow().then(function () {
    ...
    var windowFocused = function () {
      ...
      return setTimeout(() => document.querySelector('atom-workspace').focus(), 0)
    }
    window.addEventListener('focus', windowFocused)
    return ipcRenderer.on('environment', (event, env) => updateProcessEnv(env))
  })
}
