...
`
  ...
  getStylesheet () {
    ...
    for (const { selector, properties } of Array.from(this.getRulesets())) {
      lines.push(`${selector} {`)
      for (const name in properties) { const value = properties[name]; lines.push(`  ${name}: ${value};`) }
      lines.push('}\n')
    }
    return lines.join('\n')
  }
...
atom-text-editor.is-focused .line.cursor-line',
  ...
  buildScopeSelectorRulesets (scopeSelectorSettings) {
    return (() => {
      const result = []
      for (const { name, scope, settings } of Array.from(scopeSelectorSettings)) {
        if (!scope) { continue }
