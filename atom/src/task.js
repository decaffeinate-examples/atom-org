  ...
  Task = class Task {
    ...
    constructor (taskPath) {
      ...
      this.on('task:deprecations', function (deprecations) {
        for (const deprecation of Array.from(deprecations)) { Grim.addSerializedDeprecation(deprecation) }
      })
    ...
    start (...args1) {
      const adjustedLength = Math.max(args1.length, 1); const args = args1.slice(0, adjustedLength - 1); const callback = args1[adjustedLength - 1]
