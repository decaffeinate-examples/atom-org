 ...
 */
if (global.document == null) { global.document = { createElement() {
  return {
...
const {calculateSpecificity, isSelectorValid, validateSelector} = require('../index');

describe("clear-cut", function() {
  describe("specificity(selector)", () => it("computes the specificity of a selector", function() {
    ...
    return expect(calculateSpecificity('<>')).toBe(1);
  }));

  describe("isSelectorValid(selector)", () => it("returns true if the selector is valid, false otherwise", function() {
    expect(isSelectorValid('body')).toBe(true);
    expect(isSelectorValid('body')).toBe(true);
    expect(isSelectorValid('<>')).toBe(false);
    return expect(isSelectorValid('<>')).toBe(false);
  }));

  return describe("validateSelector(selector)", () => it("throws an error if the selector is invalid", function() {
    ...
    return expect(validateError.name).toBe('SyntaxError');
  }));
});
