...
`
  ...
  describe('::readFileSync(filePath)', function () {
    ...
    it('returns the compiled CSS for a given Less file path', () => expect(css).toBe(`\
body {
a: 1;
b: 2;
c: 3;
...
`
    ...
    it('relativizes cache paths based on the configured resource path', function () {
      ...
      cache2.readFileSync(join(fixturesDir, 'imports.less'))
      return expect(fs.existsSync(join(cache2.importsCacheDir, 'content', 'imports.json'))).toBeTruthy()
    })
  ...
  describe('when providing a resource path and less sources by relative file path', () => it("reads from the provided sources first, and falls back to reading from disk if a valid source isn't available", function () {
...
',
    ...
    return expect(cache3.readFileSync(join(fixturesDir, 'imports.less'))).toBe(`\
some-selector {
prop-1: 1;
prop-2: 2;
prop-3: "changed";
...
`)
  ...
  return describe('when providing a resource path and import files by relative file path', () => it("reads from the provided file paths first, and falls back to reading from disk if a valid file path isn't available", function () {
    const cacheDir = temp.mkdirSync()
    const cache1 = new LessCache({
      cacheDir,
      importPaths: [join(fixturesDir, 'imports-1'), join(fixturesDir, 'imports-2')],
