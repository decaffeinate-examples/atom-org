...
class Digester {
  ...
  digest (metadata) {
    const classes = {}
    for (const packageObject of Array.from(metadata)) {
      ...
      for (const filename in files) {
        ...
        for (const row in fileData.objects) {
          const columnsObj = fileData.objects[row]
          for (const column in columnsObj) {
  ...
  digestClass (classEntity) {
    ...
    for (const section of Array.from(sections)) {
      for (const method of Array.from(classMethods.concat(instanceMethods).concat(classProperties).concat(instanceProperties))) {
  ...
  */
  ...
  extractEntities (sections, entityPositions, entityType) {
    const entities = []
    for (const entityPosition of Array.from(entityPositions)) {
  ...
  filterSectionsForRowRange (startRow, endRow) {
    const sections = []
    for (const section of Array.from(this.current.sections)) {
