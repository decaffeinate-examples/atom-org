...
describe("CommandInstaller on #darwin", function() {
  let [installer, resourcesPath, installationPath, atomBinPath, apmBinPath] = Array.from([]);
  ...
  beforeEach(function() {
    ...
    return spyOn(CommandInstaller.prototype, 'getInstallDirectory').andReturn(installationPath);
  });

  afterEach(() => temp.cleanupSync());

  it("shows an error dialog when installing commands interactively fails", function() {
    ...
    return expect(appDelegate.confirm).toHaveBeenCalledWith({
      message: "Failed to install shell commands",
      detailedMessage: "another error"
    });
  });

  it("shows a success dialog when installing commands interactively succeeds", function() {
    const appDelegate = jasmine.createSpyObj("appDelegate", ["confirm"]);
    installer = new CommandInstaller(appDelegate);
    installer.initialize("2.0.2");
  ...
  describe("when using a stable version of atom", function() {
    beforeEach(function() {
      installer = new CommandInstaller();
      return installer.initialize("2.0.2");
    });

    it("symlinks the atom command as 'atom'", function() {
      const installedAtomPath = path.join(installationPath, 'atom');

      expect(fs.isFileSync(installedAtomPath)).toBeFalsy();

      waitsFor(done => installer.installAtomCommand(false, done));
      ...
      return runs(function() {
        ...
        return expect(fs.isFileSync(path.join(installationPath, 'atom-beta'))).toBe(false);
      });
    });

    return it("symlinks the apm command as 'apm'", function() {
      const installedApmPath = path.join(installationPath, 'apm');

      expect(fs.isFileSync(installedApmPath)).toBeFalsy();

      waitsFor(done => installer.installApmCommand(false, done));
      ...
      return runs(function() {
        ...
        return expect(fs.isFileSync(path.join(installationPath, 'apm-beta'))).toBe(false);
      });
    });
  });

  return describe("when using a beta version of atom", function() {
    beforeEach(function() {
      installer = new CommandInstaller();
      return installer.initialize("2.2.0-beta.0");
    });

    it("symlinks the atom command as 'atom-beta'", function() {
      ...
      return runs(function() {
        expect(fs.realpathSync(installedAtomPath)).toBe(fs.realpathSync(atomBinPath));
        expect(fs.isExecutableSync(installedAtomPath)).toBe(true);
        return expect(fs.isFileSync(path.join(installationPath, 'atom'))).toBe(false);
      });
    });

    return it("symlinks the apm command as 'apm-beta'", function() {
      ...
      return runs(function() {
        expect(fs.realpathSync(installedApmPath)).toBe(fs.realpathSync(apmBinPath));
        expect(fs.isExecutableSync(installedApmPath)).toBeTruthy();
        return expect(fs.isFileSync(path.join(installationPath, 'apm'))).toBe(false);
      });
    });
  });
});
