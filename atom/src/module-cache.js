...
var loadDependencies = function (modulePath, rootPath, rootMetadata, moduleCache) {
  ...
  for (const childPath of Array.from(fs.listSync(path.join(modulePath, 'node_modules')))) {
...
exports.add = function (directoryPath, metadata) {
  ...
  for (const dependency of Array.from(cacheToAdd.dependencies != null ? cacheToAdd.dependencies : [])) {
  ...
  for (const entry of Array.from(cacheToAdd.folders != null ? cacheToAdd.folders : [])) {
    for (const folderPath of Array.from(entry.paths)) {
      if (folderPath) {
