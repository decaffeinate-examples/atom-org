...
`
    ...
    it('tokenizes @if', function() {
      const {tokens} = grammar.tokenizeLine('@if $var == true');
      ...
      return expect(tokens[8]).toEqual({value: 'true', scopes: ['source.sass', 'meta.at-rule.if.sass', 'support.constant.property-value.css.sass']});
  });

    it('tokenizes @else if', function() {
      const {tokens} = grammar.tokenizeLine('@else if $var == false');
      ...
      return expect(tokens[7]).toEqual({value: 'false', scopes: ['source.sass', 'meta.at-rule.else.sass', 'support.constant.property-value.css.sass']});
  });
    ...
    it('tokenizes @each', function() {
      const {tokens} = grammar.tokenizeLine('@each $item in $list');
      ...
      return expect(tokens[9]).toEqual({value: 'list', scopes: ['source.sass', 'meta.at-rule.each.sass', 'meta.variable-usage.sass', 'variable.other.sass']});
  });
      // 'in' tested in operators

    it('tokenizes @include or \'+\'', function() {
      ...
      return expect(tokens[1]).toEqual({value: 'mixin-name', scopes: ['source.sass', 'meta.function.include.sass', 'variable.other.sass']});
  });
...
`
    ...
    return it('tokenizes @at-root', function() {
      const tokens = grammar.tokenizeLines(`\
.class
  @at-root
    #id\
`
  ...
  return describe('operators', function() {
    ...
    return it('correctly tokenizes control operators', function() {
