...
module.exports = (Referencer = class Referencer {
  ...
  getIncludedMethods (clazz) {
    ...
    for (const mixin of Array.from((clazz.doc != null ? clazz.doc.includeMixins : undefined) || [])) {
  ...
  getExtendedMethods (clazz) {
    ...
    for (const mixin of Array.from((clazz.doc != null ? clazz.doc.extendMixins : undefined) || [])) {
      result[mixin] = this.resolveMixinMethods(mixin)
    }
  ...
  getConcernMethods (clazz) {
    ...
    for (const mixin of Array.from((clazz.doc != null ? clazz.doc.concerns : undefined) || [])) {
      result[mixin] = this.resolveMixinMethods(mixin)
    }
  ...
  linkType (text, path) {
    ...
    for (const clazz of Array.from(this.classes)) {
  ...
  resolveDoc (data, entity, path) {
    if (data.doc) {
      ...
      if (data.doc.see) {
        for (const see of Array.from(data.doc.see)) {
      ...
      for (const returnValue of Array.from(data.doc.returnValue != null ? data.doc.returnValue : [])) {
