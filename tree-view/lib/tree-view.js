...
(TreeView = class TreeView {
  constructor (state) {
    ...
    if ((state.selectedPaths != null ? state.selectedPaths.length : undefined) > 0) {
      for (const selectedPath of Array.from(state.selectedPaths)) { this.selectMultipleEntries(this.entryForPath(selectedPath)) }
    } else {
  ...
  serialize () {
    return {
      directoryExpansionStates: new function (roots) {
        for (const root of Array.from(roots)) { this[root.directory.path] = root.directory.serializeExpansionState() }
        return this
      }(this.roots),
  ...
  destroy () {
    for (const root of Array.from(this.roots)) { root.directory.destroy() }
  ...
  updateRoots (expansionStates) {
    ...
    if (projectPaths.length > 0) {
      ...
      this.roots = (() => {
        const result = []
        for (const projectPath of Array.from(projectPaths)) {
          var left
  ...
  revealActiveFile (options) {
    ...
    return promise.then(() => {
      ...
      return (() => {
        const result = []
        for (const pathComponent of Array.from(activePathComponents)) {
  ...
  entryForPath (entryPath) {
    ...
    for (const entry of Array.from(this.list.querySelectorAll('.entry'))) {
  ...
  collapseDirectory (isRecursive, allDirectories) {
    ...
    if (allDirectories) {
      for (const root of Array.from(this.roots)) { root.collapse(true) }
      return
    }
  ...
  removeSelectedEntries () {
    ...
    return atom.confirm({
      ...
      if (response === 0) { // Move to Trash
        ...
        for (const selectedPath of Array.from(selectedPaths)) {
  ...
  pasteEntries () {
    ...
    return (() => {
      const result = []
      for (const initialPath of Array.from(initialPaths)) {
  ...
  deselect (elementsToDeselect) {
    ...
    for (const selected of Array.from(elementsToDeselect)) { selected.classList.remove('selected') }
    return undefined
  }
  ...
  selectContinuousEntries (entry, deselectOthers) {
    ...
    if (parentContainer === currentSelectedEntry.parentElement) {
      ...
      for (const element of Array.from(elements)) { element.classList.add('selected') }
    }
  ...
  onDragStart (e) {
    ...
    if (entry = e.target.closest('.entry')) {
      ...
      const initialPaths = []
      for (const target of Array.from(this.getSelectedEntries())) {
  ...
  onDrop (e) {
    ...
    if (entry = e.target.closest('.entry.directory')) {
      ...
      return (() => {
        const result2 = []
        for (entry of Array.from(e.dataTransfer.files)) {
