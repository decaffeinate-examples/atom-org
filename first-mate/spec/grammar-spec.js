...
describe("Grammar tokenization", function() {
  let [grammar, registry] = Array.from([]);
  ...
  beforeEach(function() {
    ...
    return loadGrammarSync('python-regex.cson');
  });

  describe("when the registry is empty", () => it("allows injections into the null grammar", function() {
    registry = new GrammarRegistry();
    ...
    return expect(tokens[0].scopes).toEqual(['text.plain.null-grammar', 'markup.underline.link.http.hyperlink']);
}));

  describe("Registry::loadGrammarSync", () => it("returns a grammar for the file path specified", function() {
    ...
    return expect(tokens[3].scopes).toEqual(['source.hello', 'suffix.hello', 'emphasis.hello']);
}));

  describe('::tokenizeLines(text)', () => describe('when the text is empty', () => it('returns a single line with a single token which has the global scope', function() {
    grammar = registry.grammarForScopeName('source.coffee');
    const lines = grammar.tokenizeLines('');
    return expect(lines).toEqual([[{value: '',  scopes: ['source.coffee']}]]);
})));

  describe("::tokenizeLine(line, ruleStack)", function() {
    describe("when the entire line matches a single pattern with no capture groups", () => it("returns a single token with the correct scope", function() {
      grammar = registry.grammarForScopeName('source.coffee');
      const {line, tags} = grammar.tokenizeLine("return");

      return expect(registry.decodeTokens(line, tags)).toEqual([
        {value: 'return', scopes: ['source.coffee', 'keyword.control.coffee']}
      ]);
  }));

    describe("when the entire line matches a single pattern with capture groups", () => it("returns a single token with the correct scope", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line doesn't match any patterns", () => it("returns the entire line as a single simple token with the grammar's scope", function() {
      const textGrammar = registry.grammarForScopeName('text.plain');
      const {line, tags} = textGrammar.tokenizeLine("abc def");
      const tokens = registry.decodeTokens(line, tags);
      return expect(tokens.length).toBe(1);
    }));

    describe("when the line matches multiple patterns", () => it("returns multiple tokens, filling in regions that don't match patterns with tokens in the grammar's global scope", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line matches a pattern with optional capture groups", () => it("only returns tokens for capture groups that matched", function() {
      grammar = registry.grammarForScopeName('source.coffee');
      ...
      expect(tokens[1].value).toBe(" ");
      return expect(tokens[2].value).toBe("Quicksort");
    }));

    describe("when the line matches a rule with nested capture groups and lookahead capture groups beyond the scope of the overall match", () => it("creates distinct tokens for nested captures and does not return tokens beyond the scope of the overall capture", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line matches a pattern that includes a rule", () => it("returns tokens based on the included rule", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line is an interpolated string", () => it("returns the correct tokens", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line has an interpolated string inside an interpolated string", () => it("returns the correct tokens", function() {
      grammar = registry.grammarForScopeName('source.coffee');
    ...
    describe("when the line is empty", () => it("returns a single token which has the global scope", function() {
      grammar = registry.grammarForScopeName('source.coffee');
      const {line, tags} = grammar.tokenizeLine('');
      return expect(registry.decodeTokens(line, tags)).toEqual([{value: '',  scopes: ["source.coffee"]}]);
  }));

    describe("when the line matches no patterns", () => it("does not infinitely loop", function() {
      grammar = registry.grammarForScopeName('text.plain');
      const {line, tags} = grammar.tokenizeLine('hoo');
      return expect(registry.decodeTokens(line, tags)).toEqual([{value: 'hoo',  scopes: ["text.plain", "meta.paragraph.text"]}]);
  }));

    describe("when the line matches a pattern with a 'contentName'", () => it("creates tokens using the content of contentName as the token name", function() {
      grammar = registry.grammarForScopeName('text.plain');
      ...
      return expect(tokens[1].scopes).toEqual(["source.test", "all", "middle"]);
  }));

    describe("when the line matches a pattern with no `name` or `contentName`", () => it("creates tokens without adding a new scope", function() {
      ...
      return expect(tokens[3]).toEqual({value: 'ook', scopes: ["source.ruby", "string.quoted.other.literal.lower.ruby"]});
  }));

    describe("when the line matches a begin/end pattern", function() {
      it("returns tokens based on the beginCaptures, endCaptures and the child scope", function() {
        grammar = registry.grammarForScopeName('source.coffee');
        ...
        return expect(tokens[2]).toEqual({value: "'''", scopes: ['source.coffee', 'string.quoted.heredoc.coffee', 'punctuation.definition.string.end.coffee']});
    });

      describe("when the pattern spans multiple lines", () => it("uses the ruleStack returned by the first line to parse the second line", function() {
        let line2, tags2;
        grammar = registry.grammarForScopeName('source.coffee');
        ...
        return expect(secondTokens[1]).toEqual({value: "'''", scopes: ['source.coffee', 'string.quoted.heredoc.coffee', 'punctuation.definition.string.end.coffee']});
    }));

      describe("when the pattern contains sub-patterns", () => it("returns tokens within the begin/end scope based on the sub-patterns", function() {
        grammar = registry.grammarForScopeName('source.coffee');
        ...
        return expect(tokens[3]).toEqual({value: '"""', scopes: ['source.coffee', 'string.quoted.double.heredoc.coffee', 'punctuation.definition.string.end.coffee']});
    }));

      describe("when applyEndPatternLast flag is set in a pattern", () => it("applies end pattern after the other patterns", function() {
...
`
        ...
        return expect(lines[4][3]).toEqual({value: "excentricSyntax }", scopes: ['source.apply-end-pattern-last', 'normal-env']});
    }));

      describe("when the end pattern contains a back reference", function() {
        it("constructs the end rule based on its back-references to captures in the begin rule", function() {
          grammar = registry.grammarForScopeName('source.ruby');
          ...
          return expect(tokens[3]).toEqual({value: ',',  scopes: ["source.ruby", "punctuation.separator.object.ruby"]});
      });

        return it("allows the rule containing that end pattern to be pushed to the stack multiple times", function() {
          grammar = registry.grammarForScopeName('source.ruby');
          ...
          return expect(tokens[11]).toEqual({value: ' damn.', scopes: ["source.ruby","comment.line.number-sign.ruby"]});
      });
    });

      return describe("when the pattern includes rules from another grammar", function() {
        describe("when a grammar matching the desired scope is available", function() {
          it("parses tokens inside the begin/end patterns based on the included grammar's rules", function() {
            ...
            return expect(tokens[22]).toEqual({value: '>', scopes: ["text.html.ruby","meta.tag.block.any.html","punctuation.definition.tag.end.html"]});
        });

          it("updates the grammar if the included grammar is updated later", function() {
            loadGrammarSync('html-rails.json');
            loadGrammarSync('ruby-on-rails.json');

            grammar = registry.grammarForScopeName('text.html.ruby');
            const grammarUpdatedHandler = jasmine.createSpy("grammarUpdatedHandler");
            ...
            return expect(tokens[13].value).toBe("select");
          });

          return it("supports including repository rules from the other grammar", function() {
            ...
            return expect(updateCallback.callCount).toBe(1);
          });
        });

        return describe("when a grammar matching the desired scope is unavailable", () => it("updates the grammar if a matching grammar is added later", function() {
          ...
          expect(tokens[8]).toEqual({value: '>', scopes: ["text.html.ruby","meta.tag.block.any.html","punctuation.definition.tag.end.html"]});
          expect(tokens[9]).toEqual({value: '<%=', scopes: ["text.html.ruby","source.ruby.rails.embedded.html","punctuation.section.embedded.ruby"]});
          return expect(tokens[10]).toEqual({value: ' ', scopes: ["text.html.ruby","source.ruby.rails.embedded.html"]});
      }));
    });
  });

    it("can parse a grammar with newline characters in its regular expressions (regression)", function() {
      ...
      return expect(tokens[2].value).toBe("");
    });

    it("can parse multiline text using a grammar containing patterns with newlines", function() {
      ...
      return expect(lines[1][2]).not.toBeDefined();
    });

    it("does not loop infinitely (regression)", function() {
      grammar = registry.grammarForScopeName('source.js');
      let {line, tags, ruleStack} = grammar.tokenizeLine("// line comment");
      return ({line, tags, ruleStack} = grammar.tokenizeLine(" // second line comment with a single leading space", ruleStack));
    });

    it("can parse a grammar that captures the same text multiple times (regression)", function() {
      ...
      return expect(lines[0][2]).toEqual({value: 'x', scopes: ['abcabx', 'abx']});
  });

    describe("when inside a C block", function() {
      beforeEach(function() {
        loadGrammarSync('c.json');
        loadGrammarSync('c-plus-plus.json');
        return grammar = registry.grammarForScopeName('source.c');
      });

      it("correctly parses a method. (regression)", function() {
        const {line, tags, ruleStack} = grammar.tokenizeLine("if(1){m()}");
        const tokens = registry.decodeTokens(line, tags);
        return expect(tokens[5]).toEqual({value: "m", scopes: ["source.c", "meta.block.c", "meta.function-call.c", "support.function.any-method.c"]});
    });

      return it("correctly parses nested blocks. (regression)", function() {
        ...
        return expect(tokens[10]).toEqual({value: "m", scopes: ["source.c", "meta.block.c", "meta.block.c", "meta.function-call.c", "support.function.any-method.c"]});
    });
  });

    describe("when the grammar can infinitely loop over a line", () => it("aborts tokenization", function() {
      spyOn(console, 'error');
    ...
    describe("when a grammar has a pattern that has back references in the match value", () => it("does not special handle the back references and instead allows oniguruma to resolve them", function() {
      ...
      const tokens = registry.decodeTokens(line, tags);
      return expect(tokens[9]).toEqual({value: "-moz-selector", scopes: ["source.css.scss", "meta.property-list.scss", "meta.property-name.scss"]});
  }));

    describe("when a line has more tokens than `maxTokensPerLine`", () => it("creates a final token with the remaining text and resets the ruleStack to match the begining of the line", function() {
      grammar = registry.grammarForScopeName('source.js');
      ...
      return expect(scopes.length).toBe(0);
    }));

    describe("when a grammar has a capture with patterns", function() {
      it("matches the patterns and includes the scope specified as the pattern's match name", function() {
        ...
        return expect(tokens[6].scopes).toEqual(["text.html.php", "meta.embedded.line.php", "source.php", "meta.function.php", "storage.type.function.php"]);
    });

      return it("ignores child captures of a capture with patterns", function() {
        grammar = loadGrammarSync('nested-captures.cson');
        const {line, tags} = grammar.tokenizeLine("ab");
        const tokens = registry.decodeTokens(line, tags);

        expect(tokens[0].value).toBe("ab");
        return expect(tokens[0].scopes).toEqual(["nested", "text", "a"]);
    });
  });

    describe("when the grammar has injections", function() {
      it("correctly includes the injected patterns when tokenizing", function() {
        grammar = registry.grammarForScopeName('text.html.php');
        ...
        return expect(tokens[17].scopes).toEqual(["text.html.php", "meta.tag.block.any.html", "entity.name.tag.block.any.html"]);
    });

      it("updates injections when grammars that the injection patterns use are updated", function() {
        ...
        return expect(tokens[1]).toEqual({value: 'SELECT', scopes: ['source.sql-injection', 'string', 'meta.embedded.sql', 'keyword.other.DML.sql']});
    });

      return it("gives lower priority to them than other matches", function() {
        ...
        expect(tokens[3].value).toBe("<");
        return expect(tokens[3].scopes).toEqual(["text.html.php2", "source.js.embedded.html", "keyword.operator.js"]);
    });
  });

    describe("when the grammar has prefixed injections", () => it("correctly prioritizes them when tokenizing", function() {
      grammar = registry.grammarForScopeName('text.html.php');
      ...
      return expect(tokens[17].scopes).toEqual(["text.html.php", "source.js.embedded.html", "entity.name.tag.script.html"]);
  }));

    describe("when the grammar has an injection selector", function() {
      ...
      return it("gives lower priority to them than other matches", function() {
        ...
        return expect(tokens[0].scopes).toEqual(["source.js", "comment.block.html.js", "punctuation.definition.comment.html.js"]);
    });
  });

    describe("when the grammar has a prefixed injection selector", () => it("correctly prioritizes them when tokenizing", function() {
      ...
      return expect(tokens[0].scopes).toEqual(["source.js", "should-be-matched.prefixed.injection-selector"]);
  }));

    describe("when the grammar's pattern name has a group number in it", () => it("replaces the group number with the matched captured text", function() {
      grammar = loadGrammarSync('hyperlink.json');
      const {line, tags} = grammar.tokenizeLine("https://github.com");
      const tokens = registry.decodeTokens(line, tags);
      return expect(tokens[0].scopes).toEqual(["text.hyperlink", "markup.underline.link.https.hyperlink"]);
  }));

    describe("when the position doesn't advance and rule includes $self and matches itself", () => it("tokenizes the entire line using the rule", function() {
      ...
      return expect(tokens[0].scopes).toEqual(["source.forever", "text"]);
  }));

    describe("${capture:/command} style pattern names", () => it("replaces the number with the capture group and translates the text", function() {
      ...
      return expect(tokens[2].scopes).toEqual(["source.ruby", "comment.line.number-sign.ruby", "storage.type.class.todo"]);
  }));

    return describe("$number style pattern names", function() {
      it("replaces the number with the capture group and translates the text", function() {
        ...
        return expect(tokens[1].scopes).toEqual(["source.makefile", "meta.scope.conditional.makefile", "meta.scope.condition.makefile"]);
    });

      it("removes leading dot characters from the replaced capture index placeholder", function() {
        loadGrammarSync('makefile.json');
        grammar = registry.grammarForScopeName('source.makefile');
        ...
        return expect(tokens[0].value).toEqual(".PHONY");
      });

      return it("replaces all occurences of capture index placeholders", function() {
        ...
        return expect(tokens[2].scopes).toEqual(["scope-names-with-placeholders"]);
    });
  });
});

  describe("language-specific integration tests", function() {
    let lines = null;

    describe("Git commit messages", function() {
      beforeEach(function() {
        grammar = loadGrammarSync('git-commit.json');
        return lines = grammar.tokenizeLines(`\
...
`
      ...
      it("correctly parses a long line", function() {
        const tokens = lines[0];
        expect(tokens[0].value).toBe("longggggggggggggggggggggggggggggggggggggggggggggggg");
        return expect(tokens[0].scopes).toEqual(["text.git-commit", "meta.scope.message.git-commit", "invalid.deprecated.line-too-long.git-commit"]);
    });

      return it("correctly parses the number sign of the first comment line", function() {
        const tokens = lines[1];
        expect(tokens[0].value).toBe("#");
        return expect(tokens[0].scopes).toEqual(["text.git-commit", "meta.scope.metadata.git-commit", "comment.line.number-sign.git-commit", "punctuation.definition.comment.git-commit"]);
    });
  });
...
`
      ...
      it("correctly parses the first include line", function() {
        const tokens = lines[0];
        expect(tokens[0].value).toBe("#");
        expect(tokens[0].scopes).toEqual(["source.c++", "meta.preprocessor.c.include"]);
        expect(tokens[1].value).toBe('include');
        return expect(tokens[1].scopes).toEqual(["source.c++", "meta.preprocessor.c.include", "keyword.control.import.include.c"]);
    });

      return it("correctly parses the second include line", function() {
        const tokens = lines[1];
        expect(tokens[0].value).toBe("#");
        expect(tokens[0].scopes).toEqual(["source.c++", "meta.preprocessor.c.include"]);
        expect(tokens[1].value).toBe('include');
...
`
      ...
      it("correctly parses variable type when it is a built-in Cocoa class", function() {
        const tokens = lines[1];
        expect(tokens[0].value).toBe("NSString");
        return expect(tokens[0].scopes).toEqual(["source.objc++", "meta.function.c", "meta.block.c", "support.class.cocoa"]);
    });

      it("correctly parses the semicolon at the end of the line", function() {
        const tokens = lines[1];
        const lastToken = _.last(tokens);
        expect(lastToken.value).toBe(";");
        return expect(lastToken.scopes).toEqual(["source.objc++", "meta.function.c", "meta.block.c"]);
    });

      return it("correctly parses the string characters before the escaped character", function() {
        const tokens = lines[1];
        expect(tokens[2].value).toBe('@"');
        return expect(tokens[2].scopes).toEqual(["source.objc++", "meta.function.c", "meta.block.c", "string.quoted.double.objc", "punctuation.definition.string.begin.objc"]);
    });
  });
    ...
    describe("Java", function() {
      beforeEach(function() {
        ...
        return grammar = registry.grammarForScopeName('source.java');
      });

      it("correctly parses single line comments", function() {
...
`
        ...
        return expect(tokens[1].value).toEqual('comment');
      });

      return it("correctly parses nested method calls", function() {
        ...
        return expect(lastToken.value).toEqual(';');
      });
    });

    describe("HTML (Ruby - ERB)", function() {
      it("correctly parses strings inside tags", function() {
        ...
        return expect(tokens[4].scopes).toEqual(["text.html.erb", "meta.embedded.line.erb", "source.ruby", "string.quoted.double.ruby", "punctuation.definition.string.end.ruby"]);
    });

      return it("does not loop infinitely on <%>", function() {
        loadGrammarSync('html-rails.json');
        loadGrammarSync('ruby-on-rails.json');
        ...
        expect(tokens.length).toBe(1);
        expect(tokens[0].value).toEqual('<%>');
        return expect(tokens[0].scopes).toEqual(["text.html.erb"]);
    });
  });

    describe("Unicode support", function() {
      describe("Surrogate pair characters", () => it("correctly parses JavaScript strings containing surrogate pair characters", function() {
        grammar = registry.grammarForScopeName('source.js');
      ...
      describe("when the line contains unicode characters", () => it("correctly parses tokens starting after them", function() {
        ...
        return expect(tokens[6].scopes).toEqual(["source.json", "meta.structure.dictionary.json", "meta.structure.dictionary.value.json", "constant.numeric.json"]);
    }));

      return describe("when the line contains emoji characters", () => it("correctly terminates quotes & parses tokens starting after them", function() {
        grammar = registry.grammarForScopeName('source.js');
        ...
        return expect(withEmojiTokens[7].scopes).toEqual([ 'source.js', 'punctuation.terminator.statement.js' ]);
    }));
  });

    describe("python", function() {
      it("parses import blocks correctly", function() {
        ...
        return expect(line2[2].scopes).toEqual(["source.python"]);
    });

      return it("closes all scopes opened when matching rules within a capture", function() {
        grammar = registry.grammarForScopeName('source.python');
    ...
    describe("HTML", function() {
      describe("when it contains CSS", () => it("correctly parses the CSS rules", function() {
...
`
        ...
        return expect(line4[4].scopes).toEqual([
          "text.html.basic",
      ...
      return describe("when it contains inline CSS", () => it("correctly stops parsing CSS", function() {
        loadGrammarSync('css.cson');
        ...
        return expect(tokens[10]).toEqual({value: "</", scopes: [
          'text.html.basic.css',
          'meta.tag.inline.any.html',
    ...
    describe("Latex", () => it("properly emits close tags for scope names containing back-references", function() {
    ...
    return describe("Thrift", () => it("doesn't loop infinitely when the same rule is pushed or popped based on a zero-width match", function() {
...
`
  ...
  describe("when the position doesn't advance", () => it("logs an error and tokenizes the remainder of the line", function() {
    spyOn(console, 'error');
    ...
    expect(tokens.length).toBe(1);
    expect(tokens[0].value).toEqual('test');
    return expect(tokens[0].scopes).toEqual(['source.loops']);
}));

  describe("when the injection references an included grammar", () => it("adds a pattern for that grammar", function() {
    loadGrammarSync("injection-with-include.cson");
    grammar = registry.grammarForScopeName("test.injections");
    expect(grammar).not.toBeNull();
    return expect(grammar.includedGrammarScopes).toEqual(['text.plain']);
}));

  return describe("when the grammar is activated/deactivated", () => it("adds/removes it from the registry", function() {
    ...
    grammar.deactivate();
    return expect(registry.grammarForScopeName('test-activate')).toBeUndefined();
  }));
});
