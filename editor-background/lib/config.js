...
'
    ...
    loadSettings () {
      ...
      for (let index = 0, end = this.tabs.length - 1, asc = end >= 0; asc ? index <= end : index >= end; asc ? index++ : index--) {
        (index => {
    ...
    activateTab (index) {
      ...
      for (let i = 0, end = this.tabs.length - 1, asc = end >= 0; asc ? i <= end : i >= end; asc ? i++ : i--) {
        (i => {
          if (i === index) {
      ...
      for (let j = 0, end1 = this.tabsContent.length - 1, asc1 = end1 >= 0; asc1 ? j <= end1 : j >= end1; asc1 ? j++ : j--) {
        (j => {
          if (j === index) {
