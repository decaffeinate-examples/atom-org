...
(Delegator = class Delegator extends Mixin {
  static delegatesProperties(...args) {
    const adjustedLength = Math.max(args.length, 1), propertyNames = args.slice(0, adjustedLength - 1), {toProperty, toMethod} = args[adjustedLength - 1];
  ...
  static delegatesMethods(...args) {
    const adjustedLength = Math.max(args.length, 1), methodNames = args.slice(0, adjustedLength - 1), {toProperty, toMethod} = args[adjustedLength - 1];
