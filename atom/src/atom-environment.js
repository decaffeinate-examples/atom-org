    ...
    */
    ...
    attemptRestoreProjectStateForPaths (state, projectPaths, filesToOpen) {
      ...
      const windowIsUnused = () => {
        for (const container of Array.from(this.workspace.getPaneContainers())) {
          for (const item of Array.from(container.getPaneItems())) {
      ...
      if (windowIsUnused()) {
        ...
        if (btn === 0) {
          ...
          for (const selectedPath of Array.from(projectPaths)) { this.project.addPath(selectedPath) }
          return Promise.all(((() => {
            const result1 = []
            for (file of Array.from(filesToOpen)) {
    ...
    restoreStateIntoThisEnvironment (state) {
      ...
      for (const pane of Array.from(this.workspace.getPanes())) { pane.destroy() }
      ...
      if (foldersToAddToProject.length > 0) {
        promise = this.loadState(this.getStateKey(foldersToAddToProject)).then(state => {
          if (state && needsProjectPaths) { // only load state if this is the first path added to the project
            ...
            const promises = []
            for (const folder of Array.from(foldersToAddToProject)) { this.project.addPath(folder) }
