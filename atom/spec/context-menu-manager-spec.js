...
describe('ContextMenuManager', function () {
  ...
  return describe('::add(itemsBySelector)', function () {
    ...
    it('can add submenu items to existing menus that can be removed with the returned disposable', function () {
      const disposable1 = contextMenu.add({ '.grandchild': [{ label: 'A', submenu: [{ label: 'B', command: 'b' }] }] })
      const disposable2 = contextMenu.add({ '.grandchild': [{ label: 'A', submenu: [{ label: 'C', command: 'c' }] }] })
    ...
    it('favors the most specific / recently added item in the case of a duplicate label', function () {
      ...
      const disposable1 = contextMenu.add({ '.grandchild': [{ label: 'A', command: 'a' }] })
      const disposable2 = contextMenu.add({ '.grandchild.foo': [{ label: 'A', command: 'b' }] })
      const disposable3 = contextMenu.add({ '.grandchild': [{ label: 'A', command: 'c' }] })
      const disposable4 = contextMenu.add({ '.child': [{ label: 'A', command: 'd' }] })
    ...
    it('excludes items marked for display in devMode unless in dev mode', function () {
      const disposable1 = contextMenu.add({ '.grandchild': [{ label: 'A', command: 'a', devMode: true }, { label: 'B', command: 'b', devMode: false }] })
