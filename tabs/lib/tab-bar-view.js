...
(TabBarView = class TabBarView {
  constructor (pane, location) {
    ...
    for (const item of Array.from(this.pane.getItems())) { this.addTabForItem(item) }
  ...
  terminatePendingStates () {
    for (const tab of Array.from(this.getTabs())) {
  ...
  onDragStart (event) {
    ...
    if (typeof item.getAllowedLocations === 'function') {
      for (const location of Array.from(item.getAllowedLocations())) {
  ...
  onClick (event) {
    ...
    if ((event.which === 3) || ((event.which === 1) && (event.ctrlKey === true))) {
      ...
      if (!this.pane.isDestroyed()) { return this.pane.activate() }
    } else if (event.which === 2) {
  ...
  onMouseLeave () {
    for (const tab of Array.from(this.getTabs())) { tab.element.style.maxWidth = '' }
  }
