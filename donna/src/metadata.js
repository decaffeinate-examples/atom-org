...
module.exports = (Metadata = class Metadata {
        ...
        switch (exp.variable.base.constructor.name) {
          case 'Literal':
            ...
            if (exp.variable.properties.length > 0) {
              ...
              for (let prop of Array.from(exp.variable.properties)) {
                if (prop.name != null) {
              ...
              switch (this.defs[exp.variable.base.value].type) {
                case 'function':
                  ...
                  return Array.from(this.files).map((file) =>
                    (() => {
                      const result = [];
                      for (let method of Array.from(file.methods)) {
  ...
  evalClass(exp) {
    ...
    for (let subExp of Array.from(exp.body.expressions)) {
      switch (subExp.constructor.name) {
        ...
        case 'Value':
          ...
          for (let prototypeExp of Array.from(subExp.base.properties)) {
            var reference;
