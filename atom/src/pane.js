...
(Pane = class Pane {
  ...
  */
  ...
  observeItems (callback) {
    for (const item of Array.from(this.getItems())) { callback(item) }
  ...
  */
  ...
  addItemsToStack (itemStackIndices) {
    if (this.items.length > 0) {
      ...
      for (const itemIndex of Array.from(itemStackIndices)) {
  ...
  saveItems () {
    for (const item of Array.from(this.getItems())) {
  ...
  */
  ...
  destroy () {
    if ((this.container != null ? this.container.isAlive() : undefined) && (this.container.getPanes().length === 1)) {
      ...
      for (const item of Array.from(this.items.slice())) {
        if (typeof item.destroy === 'function') {
          item.destroy()
        }
      }
