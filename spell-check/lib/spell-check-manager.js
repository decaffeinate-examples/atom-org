...
class SpellCheckerManager {
  ...
  setGlobalArgs (data) {
    ...
    if (removeLocaleCheckers && this.localeCheckers) {
      ...
      for (const checker of Array.from(checkers)) {
  ...
  check (args, text) {
    ...
    const promises = []
    for (const checker of Array.from(this.checkers)) {
    ...
    return Promise.all(promises).then(allResults => {
      let range
      for (const results of Array.from(allResults)) {
      ...
      for (const incorrect of Array.from(incorrects)) {
  ...
  init () {
    ...
    if (this.localeCheckers === null) {
      ...
      if (this.useLocales) {
        ...
        for (const locale of Array.from(this.locales)) {
  ...
  reloadLocales () {
    if (this.localeCheckers) {
      for (const localeChecker of Array.from(this.localeCheckers)) {
