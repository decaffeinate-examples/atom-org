...
module.exports = (Class = class Class extends Node {
  ...
  constructor (node, fileName, lineMapping, options, comment) {
    ...
    try {
      ...
      if (this.doc.methods) {
        for (const method of Array.from((this.doc != null ? this.doc.methods : undefined))) { this.methods.push(new VirtualMethod(this, method, this.lineMapping, this.options)) }
      }

      let previousExp = null

      for (const exp of Array.from(this.node.body.expressions)) {
