...
(SpanSkipList = (function () {
  SpanSkipList = class SpanSkipList {
    static initClass () {
      ...
      this.prototype.probability = 0.25
    }
    ...
    buildZeroDistance () {
      if (this.zeroDistance == null) {
        ...
        for (const dimension of Array.from(this.dimensions)) { this.zeroDistance[dimension] = 0 }
      }
    ...
    incrementDistance (distance, delta) {
      ...
      for (const dimension of Array.from(this.dimensions)) { distance[dimension] += delta[dimension] }
    }
    ...
    addDistances (a, b) {
      ...
      for (const dimension of Array.from(this.dimensions)) {
    ...
    cloneObject (object) {
      const cloned = {}
      for (const key in object) { const value = object[key]; cloned[key] = value }
      return cloned
    }
  }
