  ...
  Package = class Package {
    ...
    */
    ...
    preload () {
      ...
      this.activateMenus()
      for (const settings of Array.from(this.settings)) { settings.activate() }
    ...
    activateStylesheets () {
      ...
      for (const [sourcePath, source] of Array.from(this.stylesheets)) {
        var context, match
    ...
    activateResources () {
      ...
      if (!this.grammarsActivated) {
        for (const grammar of Array.from(this.grammars)) { grammar.activate() }
    ...
    activateKeymaps () {
      ...
      for (const [keymapPath, map] of Array.from(this.keymaps)) { this.keymapDisposables.add(this.keymapManager.add(keymapPath, map, 0, validateSelectors)) }
    ...
    loadSettings () {
      ...
      return new Promise(resolve => {
        if (this.preloadedPackage && (this.packageManager.packagesCache[this.name] != null)) {
          for (const settingsPath in this.packageManager.packagesCache[this.name].settings) {
    ...
    deactivateResources () {
      for (const grammar of Array.from(this.grammars)) { grammar.deactivate() }
      for (const settings of Array.from(this.settings)) { settings.deactivate() }
...
`
    ...
    subscribeToActivationHooks () {
      ...
      for (const hook of Array.from(this.getActivationHooks())) {
        (hook => {
    ...
    getNativeModuleDependencyPaths () {
      ...
      if (this.metadata._atomModuleCache != null) {
        ...
        for (const relativeNativeModuleBindingPath of Array.from(relativeNativeModuleBindingPaths)) {
      ...
      var traversePath = nodeModulesPath => {
        try {
          for (const modulePath of Array.from(fs.listSync(nodeModulesPath))) {
    ...
    */
    ...
    getIncompatibleNativeModules () {
      ...
      for (const nativeModulePath of Array.from(this.getNativeModuleDependencyPaths())) {
        var version
        try {
