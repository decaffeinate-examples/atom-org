...
describe('TreeView', function () {
    ...
    return describe('if there are more items than can be visible in the viewport', function () {
      ...
      return it('scrolls the selected file into the visible view', function () {
        ...
        runs(function () {
          ...
          return (() => {
            const result = []
            for (let i = 0, end = entries.length, asc = end >= 0; asc ? i < end : i > end; asc ? i++ : i--) {
  ...
  describe('keyboard navigation', function () {
    ...
    describe('tree-view:collapse-all', function () {
      const expandAll = () => (() => {
        const result = []
        for (const root of Array.from(treeView.roots)) {
          root.expand(true)
          const children = root1.querySelectorAll('.directory')
    ...
    describe('tree-view:duplicate', function () {
      describe('when a file is selected', function () {
        ...
        describe('when the path is changed and confirmed', function () {
          ...
          return describe('when a file or directory already exists at the target path', () => it('shows an error message and does not close the dialog', () => runs(function () {
            fs.writeFileSync(path.join(rootDirPath, 'target.txt'), '')
            const newPath = path.join(rootDirPath, 'target.txt')
            copyDialog.miniEditor.setText(newPath)
