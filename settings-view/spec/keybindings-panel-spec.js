...
describe('KeybindingsPanel', function () {
  let [keyBindings, panel] = Array.from([])
...
`
  ...
  describe('when the key bindings change', () => it('reloads the key bindings', function () {
    keyBindings.push({ source: atom.keymaps.getUserKeymapPath(), keystrokes: 'ctrl-b', command: 'core:undo', selector: '.editor' })
  ...
  return describe('when searching key bindings', function () {
    it('find case-insensitive results', function () {
      keyBindings.push({ source: `${atom.getLoadSettings().resourcePath}${path.sep}keymaps`, keystrokes: 'F11', command: 'window:toggle-full-screen', selector: 'body' })
      atom.keymaps.emitter.emit('did-reload-keymap')
