...
const removeEventCallback = function (receiver, eventKey, callback) {
  ...
  if (typeof callback !== 'function') {
    ...
    return (() => {
      const result = []
      for (i of Array.from(oi)) {
 				result.push(eventCallbacks.get(receiver).get(eventKey).get(onceEventKey).splice(i, 1))
      }
      return result
    })()
  }
...
let projectRingId
let projectRingConfigurationWatcher
...
module.exports = Object.freeze({
  ...
  getDefaultProjectToLoadAtStartUp (_projectRingId) {
    let defaultProjectToLoadAtStartUp
    const _fs = require('fs')
    try {
  ...
  openFiles (filePathSpec, newWindow) {
    ...
    if (newWindow) {
      ...
      for (const filePath of Array.from(filePathSpec)) { promise = promise.finally(((filePath => atom.workspace.open(filePath))).bind(null, filePath)) }
    }
    return promise
  },
  ...
  findInArray (array, value, valueModFunc, extraModFuncArgs) {
    ...
    for (const val of Array.from(array)) {
  ...
  filePathIsInProject (filePath, projectRootDirectories) {
    ...
    for (const rootDirectory of Array.from(projectRootDirectories)) {
      ...
      return pane.getItems().forEach(item => {
        ...
        if (item.buffer.file) {
          ...
          if (this.findInArray(firstNonEmptyPane.getItems(), itemBufferFilePath,
            function () {
              return this.buffer && this.buffer.file && (this.buffer.file.path.toLowerCase() === itemBufferFilePath)
					 })) {
  ...
  buildPanesLayout (panesMap) {
    ...
    return axisLayoutBuildPromise = axisLayoutBuildPromise.finally(function () {
      setTimeout(() => $('atom-pane-container > atom-pane-axis atom-pane-axis, atom-pane-container > atom-pane-axis atom-pane').each(function () {
        ...
        return $this.css('flex-grow', flexGrow)
      }), 0)
